...
## Audience
{% include note.html priority='medium' note='TO DO:  Review with Partnership Team and Product Management<p>TO DO:  Provide email form to constituent product management and global partnership team</p>' %}

Currently, the RE NXT API is only available to a select group of Blackbaud partners.  Over time, as the API matures, we will open the API to a wider audience.  If you are an existing of potential Blackbaud Partner and are interested in building an integration or 3rd party product that integrates with our APIs, we encourage you to constituent our Partnership team.  If all goes well, you can apply to one of our API products within the developer portal.  For details, see our <a href="{{ '/tutorials/getting-started/' | prepend: site.baseurl }}" > Getting Started</a> tutorial. 
...
The majority of endpoints access *private* data, such as constituent data.  To access private data an application must get permission from a specific tenant's {{ site.productname }} user.  <a href="{{ '/guide/#web-api-authorization' | prepend: site.baseurl }}" > Web API Authorization</a> is done via the {{site.authorizationservicename}} at the following URL: 

<code>{{site.authorizationbaseurl}}</code>  

## Endpoints

The {{ site.productname }} Web API currently accesses the following top level resource groupings. Click on a link to read the documentation for accessing or modifying each resource:

- <a href="{{ '/guide/#constituents' | prepend: site.baseurl }}" >Constituents</a> - Manage a constituent's biographical and constituent information for a specific Blackbaud tenant which includes addresses, email addresses, phones, web addresses, name formats ...
...
<div class="table-responsive">
  <table class="table table-striped table-hover">
	...
	<tbody>
	<tr>
		<td>Retrieve a specific constituent using the <b>constituentId</b> path parameter</td>
		<td><a href="{{ site.apiconsoleconstituentget | prepend: site.devportalurl }}" >GET {{ '/constituents/(constituentId)' | prepend: site.apibaseurl }}</a></td>

	</tr>
	<tr>
		<td>Update a Constituent sing the <b>constituentId</b> path parameter</td>
		<td><a href="{{ site.apiconsoleconstituentupdate | prepend: site.devportalurl }}" >PUT {{ '/constituents/(constituentId)' | prepend: site.apibaseurl }}</a></td>
	</tr>
	<tr>
		<td>Delete a Constituent sing the <b>constituentId</b> path parameter</td>
		<td><a href="{{ site.apiconsoleconstituentdelete | prepend: site.devportalurl }}" >DELETE {{ '/constituents/(constituentId)' | prepend: site.apibaseurl }}</a></td>
	</tr>
	<tr>
		<td>Adding a Constituent</td>
		<td><a href="" >POST {{ '/constituents' | prepend: site.apibaseurl }}</a></td>
	</tr>
	<tr>
		<td>Searching for a Constituent using <b>lookupID</b> URI parameter</td>
		<td><a href="" >GET {{ '/constituents?lookupID=(value)' | prepend: site.apibaseurl }}</a></td>

	</tr>
	<tr>
		<td>Searching for a Constituent using <b>email</b> URI parameter</td>
		<td><a href="" >GET {{ '/constituents?email=(value)' | prepend: site.apibaseurl }}</a></td>
	</tr>
	<tr>
		<td>More...</td>
		<td>More...</td>
...
This process uses 2 API calls to add a constituent to  {{ site.productname }} constituent, one to determine if it already exists, and one to add it. It is fine to use this process if you are adding a fairly small number of constituents. 
...
{% include note.html priority='medium' note='Document the standard/typical HTTP request response headers returned by the API such as Content-type and Content-length <p> Engineering should work with documentation to identify and explain any significant and important HTTP headers such as Cache-control or Authorization</p>' %}
ipsum lorem
