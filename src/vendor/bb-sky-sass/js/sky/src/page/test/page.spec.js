...
describe('Page directive', function () {
    ...
    describe('bbPage', function () {
        ...
        it('raises the first view loaded event', inject(function (bbData) {

            var emitSpy,
                fakeOnFirstItemLoaded;

            spyOn(bbData, 'loadManager').and.callFake(function (options) {
                fakeOnFirstItemLoaded = options.onFirstItemLoaded;
            });
            ...
            emitSpy = spyOn(elScope, '$emit');

            fakeOnFirstItemLoaded();

            expect(emitSpy).toHaveBeenCalledWith('bbPageFirstViewLoaded');


        }));

        it('raises the page load complete event', inject(function (bbData) {

            var emitSpy,
                fakeOnLastItemLoaded;

            spyOn(bbData, 'loadManager').and.callFake(function (options) {
                fakeOnLastItemLoaded = options.onLastItemLoaded;
            });
            ...
            fakeOnLastItemLoaded("myView");

            expect(emitSpy).toHaveBeenCalledWith('bbPageLoadComplete', { viewName: "myView" });
