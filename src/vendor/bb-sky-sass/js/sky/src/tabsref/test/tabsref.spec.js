...
describe('Tab Sref directive', function () {
    ...
    beforeEach(inject(function (_$compile_, _$rootScope_, _$timeout_) {
        ...
        tabHtml = 
            '<tabset>' + 
                '<tab heading="1" bb-tab-sref="tabstate.a" active="activeTabA"></tab>' + 
                '<tab heading="2" bb-tab-sref="tabstate.b" active="activeTabB" select="tabSelectB()"></tab>' +
            '</tabset>';
        /*jslint white: false */
    }));
    ...
    it('should change state when a tab is selected', function () {
        ...
        $scope.activeTabA = true;
        $scope.$digest();

        $scope.activeTabB = true;
        $scope.$digest();
